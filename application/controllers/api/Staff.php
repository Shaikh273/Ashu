    ...
    public function staff_profile_get(){
        ...
        if(!empty($role)){
            if($role == 'Super Admin'){
                ...
                for($j = 0; $j < count($org); ++$j){
                    $org_id = $org[$j]->org_id;
                    $data['org'][$i]= $this->db->select('*')->from($this->organization)->where("org_id = '$org_id'")->get()->row();
                    $data['org'][$i]->staff = $this->db->select('*')->from($this->staff)->where("org_id = '$org_id'")->get()->result();
                }
            }else{
